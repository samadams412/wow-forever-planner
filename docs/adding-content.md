# Adding guides and blog posts

Quick reference for adding a new guide or blog post by hand. Both use the
same underlying system (filesystem `.mdx` files + `next-mdx-remote`, not
Next's `@next/mdx` file-convention routing) — the two are mirrored, separate
implementations rather than one shared template, so they can diverge later
if needed. Everything below applies identically to both unless a section
says otherwise.

## How it actually works (not a build step)

There is **no manifest, index, or generator to run**. A post/guide is just
an `.mdx` file under `content/blog/` or `content/guides/`, read at
request/build time via `fs.readdirSync`/`fs.readFileSync` + `gray-matter`
(see `lib/blog.ts` / `lib/guides.ts`). The `[slug]` route reads the matching
file and renders its body through `<MDXRemote>`.
...
Pulled directly from `lib/blog.ts` / `lib/guides.ts` and cross-checked
against `content/blog/mount-hyjal.mdx` and the other three published posts
— all four use every field below, so there are currently no fields that are
sometimes omitted in practice, even though only `status` is structurally
load-bearing.
...
| `date`      | Yes      | string `"YYYY-MM-DD"` | Drives sort order (newest first) on both listing pages. Rendered with `timeZone: "UTC"`, so pick the date, not a time. |
| `summary`   | Yes      | string            | Shown on the listing card and used as the meta description. |
| `tags`      | Yes      | string[]          | Shown as small pill badges. Can be `[]`. |
| `status`    | Yes      | `"draft"` \| `"published"` | Only `"published"` is ever rendered anywhere (see above). |
| `heroImage` | Yes      | string (root-relative path) | e.g. `"/images/blog/mount-hyjal/hero.webp"`. Rendered via the shared `<GuideImage>` component, `fill` + `object-cover`, forced 16:9. |
| `heroAlt`   | Yes      | string            | Alt text for the hero image. Write a real description — it's the only alt text `GuideImage` renders, there's no fallback. |
...
`GuideFrontmatter` in `lib/blog.ts`/`lib/guides.ts`) — omitting a field won't
error at build time (frontmatter is cast with `as`, not validated), but it
will render as `undefined` (e.g. a blank date or a broken tag list), so
treat all seven fields as required in practice.
...
(the two files are intentionally identical copies, not a shared import, so
one content type's styling can drift from the other later without a
refactor).

One custom MDX component is available in both: `<GuideImage>`, for images
inside the body (not just the hero):
...
- **Location:** one folder per post/guide, named after its slug:
  `public/images/blog/<slug>/` or `public/images/guides/<slug>/` (see
  `public/images/README.md`, which documents this same structure).
...
- **Format:** `.webp` for every existing image — stay consistent.
- **Credit line:** `<GuideImage>` hardcodes a single credit string —
  *"Image: Official World of Warcraft: Forever reveal screenshot, courtesy
  of Blizzard Entertainment"* — under every image, for both the hero and
  body images, with no per-image override. This means **`<GuideImage>` is
  only correct for official Blizzard press stills/reveal screenshots.** If
  you ever need a custom-made image (a diagram, a screenshot you took, a
  graphic you drew) that needs different credit or no credit line, don't
  reuse `<GuideImage>` as-is — it would mislabel the source. That's a real
  gap, not a solved case; flag it before forcing a non-Blizzard image
...
- **Sitemap is not automatic.** `app/sitemap.ts` has a literal `// TODO:
  ...
  published blog posts are already missing from the sitemap today.** Adding
  a new post does not add it to the sitemap; someone has to update
  `app/sitemap.ts` by hand (or someone eventually wires `getAllPosts()`/
  `getAllGuides()` into it, which hasn't happened yet).
